#!/usr/bin/env python3
"""
mininab.py — a minimal YNAB-style CLI tool (v0.9.1) with account and transfer support
"""
...
# --- Basic Setup ---
DATA = pathlib.Path.home() / ".mininab.json"
LOG_FILE = pathlib.Path.home() / ".mininab.log"
...
def cmd_cat(state: Dict[str, Any], spec: str) -> None:
    """Add a new category or subcategory using Parent:Child"""
    logging.info("Executing cmd_cat: spec=%s", spec)
    parts = spec.split(":", 1)
    if len(parts) == 2:
        parent, child = parts[0].strip(), parts[1].strip()
        if parent not in state["categories"]:
            state["categories"][parent] = {"parent": None}
            logging.info("Category '%s' added.", parent)
            print(f"Category '{parent}' added.")
        key, parent_key = f"{parent}:{child}", parent
    else:
        key, parent_key = parts[0].strip(), None
    cats = state.setdefault("categories", {})
    if key in cats:
        logging.warning("Category '%s' already exists.", key)
        print(f"Category '{key}' already exists.")
        return
    cats[key] = {"parent": parent_key}
    logging.info("Category '%s' added.", key)
    print(f"Category '{key}' added.")
